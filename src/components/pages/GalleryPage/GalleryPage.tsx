...
const GalleryPage = () => {
    ...
    try {
      ...
      const authorsWithNames = uniqueAuthorIds
        ...
        .filter(
          (
            author,
          ): author is { id: string; firstName: string; lastName: string } =>
            author !== null,
        )
  ...
  const sortPosts = (
    ...
    switch (sort) {
      ...
      case "likes,desc":
        return sorted.sort(
          (a, b) => ((b as any).likes || 0) - ((a as any).likes || 0),
        );
      case "likes,asc":
        return sorted.sort(
          (a, b) => ((a as any).likes || 0) - ((b as any).likes || 0),
        );
      default:
        return sorted;
    }
    ...
    try {
      ...
      if (authorFilter !== "all") {
        const filteredPosts = allPosts.filter(
          (post) => post.authorId === authorFilter,
        );
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.imageUrl.trim()) {
      ...
      errors.imageUrl =
        "Bitte gib eine gültige URL ein (z.B. https://example.com/bild.jpg)";
      isValid = false;
    }
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      >
        ...
        <Typography variant="subtitle1" sx={{ opacity: 0.9, mb: 3 }}>
          {totalElements} {totalElements === 1 ? "Bild" : "Bilder"} in der
          Sammlung
        </Typography>
          ...
          <DialogContent sx={{ pt: 3 }}>
            ...
            <TextField
              ...
              helperText={
                ...
                `${(formData.description || "").length}/${MAX_DESCRIPTION_LENGTH} Zeichen`
              }
            />
      ...
      {isAdmin && (
        ...
        >
          <DialogTitle sx={{ pb: 1 }}>Post löschen?</DialogTitle>
          <DialogContent>
            <Typography variant="body2" color="text.secondary">
              Bist du sicher, dass du diesen Post löschen möchtest? Diese Aktion
